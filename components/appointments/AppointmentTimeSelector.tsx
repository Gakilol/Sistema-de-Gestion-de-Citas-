...
function validarLocal(
  ...
  for (const int of intervalos) {
    if (startMin < int.fin && endMin > int.inicio) {
      return {
        valida: false,
        motivo: `${int.motivo} (${formatTime12Hour(minutesToTime(int.inicio))} - ${formatTime12Hour(minutesToTime(int.fin))})`
      };
    }
  }
...
export function AppointmentTimeSelector({
  ...
  const [intervalosOcupados, setIntervalosOcupados] = useState<OccupiedInterval[]>([]);
          ...
          {intervalosOcupados.map((int, i) => {
            ...
            return (
              <div
                ...
                title={`${int.motivo}: ${formatTime12Hour(minutesToTime(int.inicio))} - ${formatTime12Hour(minutesToTime(int.fin))}`}
              >
                <div
            ...
            if (currentPercentage >= 0 && currentPercentage <= 100) {
              return (
                <div
                  ...
                  title={`Hora actual: ${formatTime12Hour(businessNow)}`}
                >
          ...
          {hourMarks.map(h => {
            ...
            return (
              <span
                ...
                style={{ left: `${pos}%` }}
              >
                {formatTime12Hour(minutesToTime(h)).replace(':00', '')}
              </span>
            );
          })}
        </div>
        ...
        {validacionActual.valida ? (
          <div className="flex items-center justify-between gap-2 px-3.5 py-2.5 rounded-xl bg-emerald-500/10 dark:bg-emerald-950/30 border border-emerald-500/30 text-xs">
            ...
            <span className="text-emerald-700 dark:text-emerald-300 font-bold tabular-nums">
              {formatTime12Hour(selectedTime)} – {formatTime12Hour(minutesToTime(selectedMinutes + duracion))} ({duracion} min)
            </span>
          </div>
        ) : (
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between text-[10px] text-muted-foreground/60 pt-1.5 border-t border-border/20">
        <span>
          {jornada && `Jornada del estilista: ${formatTime12Hour(jornada.inicio)} – ${formatTime12Hour(jornada.fin)}`}
        </span>
        <span>
