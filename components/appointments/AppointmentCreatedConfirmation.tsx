...
interface AppointmentCreatedConfirmationProps {
  ...
  open: boolean;
  onClose: () => void;
  onEdit: (cita: CalendarAppointment) => void;
}
...
function getServicios(cita: CalendarAppointment): string[] {
  if (Array.isArray(cita.citaServicios) && cita.citaServicios.length > 0) {
    return cita.citaServicios
      .map((item: { servicio?: { nombre?: string } }) => item.servicio?.nombre)
      .filter((name: string | undefined): name is string => Boolean(name));
  }
  return cita.servicio?.nombre ? [cita.servicio.nombre] : [];
}

export function AppointmentCreatedConfirmation({ cita, open, onClose, onEdit }: AppointmentCreatedConfirmationProps) {
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-4">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <Info icon={<Clock className="size-4 text-emerald-500" />} label="Horario" value={`${formatTime12Hour(cita.hora)} – ${formatTime12Hour(fin.hora)}`} detail={`${cita.duracion} minutos`} />
            </div>
