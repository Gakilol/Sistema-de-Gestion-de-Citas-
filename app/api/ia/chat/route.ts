...
const functionDeclarations = [
  ...
  { name: 'prepareCreateAppointment', description: 'Valida disponibilidad y prepara una cita para un cliente que ya existe en Clientes. Si la hora exacta no existe, puede proponer el espacio disponible más cercano dentro de 30 minutos y siempre lo muestra antes de confirmar. Nunca registra clientes ni guarda todavía.', parameters: { type: 'OBJECT', properties: { cliente: { type: 'STRING', description: 'Nombre o teléfono de un cliente existente.' }, telefono: { type: 'STRING', description: 'Teléfono del cliente, si lo proporcionó.' }, servicio: { type: 'STRING', description: 'Nombre exacto del servicio si existen opciones parecidas.' }, profesional: { type: 'STRING' }, fecha: { type: 'STRING', description: 'Fecha exacta YYYY-MM-DD.' }, hora: { type: 'STRING', description: 'Hora preferida HH:mm; puede aproximarse al espacio disponible más cercano dentro de 30 minutos.' }, notas: { type: 'STRING' } }, required: ['cliente', 'servicio', 'fecha', 'hora'] } },
  ...
  { name: 'prepareAddClientPreference', description: 'Prepara guardar en la ficha de un cliente un corte, tinte, alergia, preferencia o nota.', parameters: { type: 'OBJECT', properties: { cliente: { type: 'STRING' }, tipo: { type: 'STRING', enum: ['CORTE', 'TINTE', 'ALERGIA', 'PREFERENCIA', 'NOTA'] }, titulo: { type: 'STRING' }, detalle: { type: 'STRING' } }, required: ['cliente', 'tipo', 'titulo', 'detalle'] } },
  { name: 'prepareWhatsAppReminder', description: 'Busca la cita de un cliente y prepara abrir un recordatorio gratuito en WhatsApp. No lo envía automáticamente.', parameters: { type: 'OBJECT', properties: { query: { type: 'STRING', description: 'Nombre o teléfono del cliente.' }, fecha: { type: 'STRING', description: 'YYYY-MM-DD; omitir significa hoy.' }, hora: { type: 'STRING', description: 'HH:mm si hay varias citas.' } }, required: ['query'] } },
];
...
La hora es flexible dentro de 30 minutos: si la solicitada no está libre, puedes proponer el espacio disponible más cercano, pero debes explicar el ajuste y dejar que el usuario lo confirme. Nunca cambies a una hora más lejana sin pedir otra elección.
...
Para iniciar, completar o cancelar una cita usa preferentemente prepareUpdateAppointmentStatusByQuery con el nombre del cliente; no le pidas identificadores al usuario. Si hay varias coincidencias, pregunta por la hora.
WhatsApp funciona abriendo un mensaje preparado con wa.me: nunca digas que fue enviado automáticamente.
