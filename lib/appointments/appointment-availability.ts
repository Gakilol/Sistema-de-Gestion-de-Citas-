...
import { validateNormalBusinessTime } from '@/lib/time-utils';
...
export function validateExactAppointmentTime(
  horaStr: string,
  duracion: number,
  jornadaInicio: string,
  jornadaFin: string,
  intervalosOcupados: OccupiedInterval[],
  permitirHorarioExtendido: boolean = false,
  turnosEmpleado?: EmployeeShift[]
...
export async function calculateAppointmentAvailability(
  empleadoId: string, 
  ...
  let turnosEmpleadoDia: EmployeeShift[] | undefined = undefined;
  ...
  const intervalosOcupados: OccupiedInterval[] = [
    ...citas.map((c: any) => ({
  ...
  for (const turno of horarioDia) {
    ...
    while (currentMinutes < endMinutes) {
      ...
      const resultado = validateExactAppointmentTime(horaString, duracionServicio, jornada.inicio, jornada.fin, intervalosOcupados, permitirHorarioExtendido, turnosEmpleadoDia);
  ...
  if (horaRequerida && /^\d{2}:\d{2}$/.test(horaRequerida)) {
    ...
    const resultado = validateExactAppointmentTime(horaRequerida, duracionServicio, jornada.inicio, jornada.fin, intervalosOcupados, permitirHorarioExtendido, turnosEmpleadoDia);
...
export async function findAppointmentConflicts(
  ...
  const conflictos: AppointmentConflict[] = [];
